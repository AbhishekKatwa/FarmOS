...
@HiltViewModel class ProductionHistoryViewModel @Inject constructor(
    state: SavedStateHandle, private val getFlock: GetFlockUseCase, private val listProduction: ListProductionUseCase
) : ViewModel() {
    private val flockId = checkNotNull<String>(state["flockId"])
    private val _state = MutableStateFlow(ProductionHistoryUiState()); val uiState = _state.asStateFlow()
    init { refresh() }
    fun refresh() = viewModelScope.launch {
        _state.update { it.copy(loading = true, error = null) }
        val flock = getFlock(flockId).getOrElse { _state.update { s -> s.copy(loading = false, error = it.message) }; return@launch }
        listProduction(flockId).fold({ values -> _state.value = ProductionHistoryUiState(flock, values, false) }, { error -> _state.value = ProductionHistoryUiState(flock, loading = false, error = error.message) })
    }
}
...
@HiltViewModel class ProductionEditorViewModel @Inject constructor(
    ...
    fun save(date: String, opening: Int, mortality: Int, culls: Int, eggs: Int, broken: Int, dirty: Int, usable: Int, rejected: Int, feed: Double, remarks: String, gradeValues: Map<String, Int>, mortalityCause: String, mortalityRemarks: String) = viewModelScope.launch {
        ...
        val mortalityRecord = if (mortality > 0 || mortalityCause.isNotBlank() || mortalityRemarks.isNotBlank()) MortalityRecord(farmId = flock.farmId, flockId = flock.id, date = date, mortalityCount = mortality, cause = mortalityCause, remarks = mortalityRemarks) else null
        val item = DailyProduction(id = current.production?.id.orEmpty(), farmId = flock.farmId, shedId = flock.shedId, flockId = flock.id, date = date, openingLiveBirds = opening, mortality = mortality, culls = culls, eggsCollected = eggs, brokenEggs = broken, dirtyEggs = dirty, usableEggs = usable, rejectedEggs = rejected, feedConsumedKg = feed, remarks = remarks, eggGrades = gradeValues.map { EggGradeEntry(it.key, it.value) }, mortalityRecord = mortalityRecord)
        _state.update { it.copy(saving = true, error = null) }
        val result = if (item.id.isBlank()) create(item) else update(item)
