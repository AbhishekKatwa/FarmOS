...
@Singleton class FeedRepositoryImpl @Inject constructor(private val db:Postgrest):FeedRepository {
 ...
 override suspend fun items(farmId:String)=runCatching{db["feed_items"].select{filter {FeedItemDto::farmId eq farmId}}.decodeList<FeedItemDto>().map{it.domain()}}
 override suspend fun purchases(itemId:String)=runCatching{db["feed_purchases"].select{filter {FeedPurchaseDto::feedItemId eq itemId}}.decodeList<FeedPurchaseDto>().map{it.domain()}}
 override suspend fun consumption(itemId:String)=runCatching{db["feed_consumption"].select{filter {FeedConsumptionDto::feedItemId eq itemId}}.decodeList<FeedConsumptionDto>().map{it.domain()}}
 override suspend fun adjustments(itemId:String)=runCatching{db["feed_adjustments"].select{filter {FeedAdjustmentDto::feedItemId eq itemId}}.decodeList<FeedAdjustmentDto>().map{it.domain()}}
