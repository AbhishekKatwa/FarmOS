...
class ProductionRepositoryImpl @Inject constructor(
    ...
    override suspend fun listByFlock(flockId: String) = runCatching {
        postgrest["production_daily"].select { filter {DailyProductionDto::flockId eq flockId }}
    ...
    override suspend fun update(production: DailyProduction) = runCatching {
        ...
        postgrest["production_daily_egg_grade_entries"].delete { filter {EggGradeEntryDto::productionDailyId eq production.id }}
        postgrest["mortality_records"].delete { filter {MortalityRecordDto::productionDailyId eq production.id} }
